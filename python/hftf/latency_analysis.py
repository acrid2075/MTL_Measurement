...
def process_file(filename):
    try:
        ...
        ticker, locate = stem.rsplit("_", 1) if "_" in stem else (stem, None)
        ...
        for i in range(1, len(signals)):
            ...
            if signals.timestamp.iloc[i] > execution_time:
                if shares_to_purchase == 2:
                    ...
                    eligible = tempdf[tempdf['timestamp'] <= execution_time]
                    if eligible.empty or eligible['ask'].dropna().empty:
                        ask_exec = sp
                    else:
                        ask_exec = eligible['ask'].dropna().iloc[-1]
                    slippage = ask_exec - sp
                    trade_count += 1
                elif shares_to_purchase == -2:
                    tempdf = filtered_df.loc[signals.index[i - 1]: signals.index[i]]
                    eligible = tempdf[tempdf['timestamp'] <= execution_time]
                    if eligible.empty or eligible['bid'].dropna().empty:
                        bid_exec = sp
                    else:
                        bid_exec = eligible['bid'].dropna().iloc[-1]
