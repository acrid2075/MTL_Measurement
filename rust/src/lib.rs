...
impl<R: Read> MsgStream<R> {
    ...
    pub fn get_locate_codes(&mut self, focused_names: Vec<&str>, date: &str) -> Result<()> {
        ...
        let file =
            std::fs::read_to_string(format!("./data/locates/bx_stocklocate_20{}.txt", { reformated }))
                .unwrap();
            ...
            for msg in feed {
                if msg.typ == Some(68) {
                    ...
                    if bids.contains_key(&orn) {
                        ...
                        //if bid_spread.get(&order[0]) == Some(&0){
                            //bid_spread.remove(&order[0]);
                        //}
                        bid_depth -= order[1];
                    ...
                    if bids.contains_key(&msg.orrf.unwrap()) {
                        ...
                        bid_spread
                            ...
                            .and_modify(|shares| *shares += msg.shares.unwrap())
                            .or_insert(msg.shares.unwrap());
                        bid_depth += 0 - (del[1]) + msg.shares.unwrap();
                    } else if asks.contains_key(&msg.orrf.unwrap()) {
                        ...
                        ask_spread
                            ...
                            .and_modify(|shares| *shares += msg.shares.unwrap())
                            .or_insert(msg.shares.unwrap());
                        ask_depth += 0 - (del[1]) + msg.shares.unwrap();
                    }
                ...
                let mut ask: u32 = 0;
                if bid_spread.len() > 0 {
                    for (k, v) in bid_spread.iter().rev() {
                        if v > &0 {
                            bid = k.clone();
                            break;
                        }
                ...
                msg.bid = Some(bid);
                msg.ask = Some(ask);
                msg.spread = Some(if ask >= bid { ask - bid } else { None });
    ...
    pub fn write_companies(&mut self, date: &str) -> Result<()> {
        std::fs::create_dir(format!("./data/lob_files/{date}"))?;
        for (loc, feed) in &self.companies {
            println!("writing to {}", loc);
            let name = self.loc_to_ticker.get(loc).unwrap();
            let mut wtr = Writer::from_path(format!("./data/lob_files/{date}/{name}.csv"))?;
            //let mut wtr = Writer::from_path(format!("./data/lob_files/{name}.csv"))?;
            for msg in feed {
                wtr.serialize(msg)?;
            }
        }
